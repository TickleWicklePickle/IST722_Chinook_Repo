...
WITH stg_genre_analysis AS (
    SELECT
        ...
        ,count(distinct(p.playlistid)) as playlists_per_genre               -- Playlist count per genre
        ...
        ,REPLACE(TO_DATE(I.invoicedate)::varchar,'-','')::int  as invoice_date_key --date key of purchase(aka invoice) date
